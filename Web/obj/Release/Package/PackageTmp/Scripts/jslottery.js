 ...
 */
 var Jslottery = (function(){
	...
	function Jslottery(opt){
		var options = {
		    scroll_dom: null,//用来滚动的dom元素
		    start_position: 0,//元素第一次开始的位置
		    stop_position: 0,
			speed:null,
			speedUp:null,
			speedDown:null,
	...
	Jslottery.prototype = {
		...
		judge_dom: function () {
		    var self = global.options;
			self.scroll_dom =  document.getElementsByClassName(self.scroll_dom);
		},
		...
		stop: function () {
		    global.options.callback({ 'status': '1', 'data': global.options.stop_position });
			global.fixs.run=1;
		},
		...
		start_scroll:function(){
			...
			scroll_json[self.scroll_dom_css] = self.scroll_dom_css_value;///Images/houzi.jpg
			for (var i = 0; i <= global.fixs.num; i++) {
				if(self.scroll_dom[i].getAttribute(self.scroll_dom_attr)==self.start_position)
				{
					...
					self.scroll_dom[i].src = scroll_json[self.scroll_dom_css];
						...
						if(self.start_position==1)
						{
							for(var k=0;k<fix.num;k++){
								if(self.scroll_dom[k].getAttribute(self.scroll_dom_attr)==fix.num){
								    self.scroll_dom[k].src = original_json[self.scroll_dom_css];
								}
							}
						}else if(self.scroll_dom[j].getAttribute(self.scroll_dom_attr)==self.start_position-1)
						{
						    self.scroll_dom[j].src = original_json[self.scroll_dom_css];
						}
					}
